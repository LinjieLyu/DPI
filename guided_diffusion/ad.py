...
class lightVarIntegrator(RBIntegrator):
    r"""
    .. _integrator-prb:
    ...
    def sample(self,
        ...
        while loop(active):
            ...
            with dr.resume_grad(when=not primal):
                Le = (β * mis)*(β * mis)* ds.emitter.eval(si)
            ...
            with dr.resume_grad(when=not primal):
                ...
                Lr_dir = (β * mis_em * bsdf_value_em)*(β * mis_em * bsdf_value_em) * em_weight
...
class BRDFVARIntegrator(RBIntegrator):
    r"""
    .. _integrator-prb:
    Path Replay Backpropagation (:monosp:`prb`)
    -------------------------------------------
    ...
    def sample(self,
        ...
        while loop(active):
            ...
            with dr.resume_grad(when=not primal):
                ...
                bsdf_value_em, bsdf_pdf_em = bsdf.eval_pdf(bsdf_ctx, si, wo, active_em)
                mis_em = dr.select(ds.delta, 1, mis_weight(ds.pdf, bsdf_pdf_em))
                Lr_dir = (β * mis_em * em_weight) *(β * mis_em * em_weight) * bsdf_value_em
