...
from util.img_utils import clear_color,to_output
...
class GaussianDiffusion:
    ...
    def diffussion(self, x_t_previous, t):
        """
        Diffuse the data for a given number of diffusion steps.

        In other words, sample from q(x_t | x_t-1).
        ...
        """
        noise = torch.randn_like(x_t_previous)
        assert noise.shape == x_t_previous.shape
        
        beta=extract_and_expand(self.betas, t, x_t_previous)
        
        img_in_est =torch.sqrt(1 - beta) * x_t_previous + \
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            if record:
                if idx % 10 == 0 or idx>990:
                    file_path = os.path.join(save_root, f"progress/x_{str(idx).zfill(4)}.png")
                    plt.imsave(file_path, clear_color(img))
                    file_path = os.path.join(save_root, f"progress/x_0_hat_{str(idx).zfill(4)}.png")
                    plt.imsave(file_path, to_output((x_0_hat+1.)/2))
    ...
    def p_mean_variance(self, model, x, t):
        if t.float().item() / self.num_timesteps<0.9:
            shift=random.randint(0,2)
            model_output = model(torch.roll(x,shift,-1), self._scale_timesteps(t))       
            model_output=torch.roll(model_output,-shift,-1)
        ...
        # model_mean, pred_xstart = torch.roll(model_mean,-shift,-1),torch.roll(pred_xstart,-shift,-1)
        # model_variance, model_log_variance =  torch.roll(model_variance,-shift,-1),torch.roll(model_log_variance,-shift,-1)
